...
abstract class IReloadRepository {
  ...
  Future<ReloadSession> createSession(
      Session session, ReloadSession reloadSession);
  Future<ReloadSession?> findSessionById(Session session, UuidValue id);
  Future<List<ReloadSession>> listSessionsByUser(Session session, int userId);
  ...
  Future<List<ReloadTest>> listTestsBySession(
      Session session, UuidValue sessionId);
  ...
  Future<List<SupplyStock>> listSuppliesByUser(Session session, int userId);
  ...
  Future<SupplyStock> adjustSupplyQuantity(
      Session session, UuidValue id, double change);
}
