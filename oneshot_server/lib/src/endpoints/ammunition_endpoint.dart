...
class AmmunitionEndpoint extends Endpoint {
  ...
  Future<List<AmmunitionStock>> getMyAmmunition(Session session) async {
    ...
    return await sl.ammunitionRepository.findByUserId(session, profile.id!);
  }

  /// Adiciona uma nova munição ao estoque (ex: compra).
  Future<AmmunitionStock> addAmmunition(Session session, AmmunitionStock ammo) async {
    final profile = await sl.getOrCreateProfileUseCase.execute(session);
    ammo.userId = profile.id;
  ...
  Future<AmmunitionStock?> adjustQuantity(Session session, UuidValue id, int change) async {
    ...
    if (existing == null || existing.userId != profile.id) {
