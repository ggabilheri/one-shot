...
class ReloadEndpoint extends Endpoint {
  ...
  Future<ReloadSession> executeReloadSession(Session session, ReloadSession reloadSession) async {
    final profile = await sl.getOrCreateProfileUseCase.execute(session);
    reloadSession.userId = profile.id;
  ...
  Future<List<ReloadSession>> getMyReloadSessions(Session session) async {
    final profile = await sl.getOrCreateProfileUseCase.execute(session);
    return await sl.reloadRepository.listSessionsByUser(session, profile.id!);
  }
  ...
  Future<ReloadTest> registerTest(Session session, ReloadTest test) async {
    ...
    final reloadSession = await sl.reloadRepository.findSessionById(session, test.reloadSessionId!);
    if (reloadSession == null || reloadSession.userId != profile.id) {
  ...
  Future<List<ReloadTest>> getTestsBySession(Session session, UuidValue sessionId) async {
  ...
  Future<SupplyStock> updateSupply(Session session, SupplyStock supply) async {
    final profile = await sl.getOrCreateProfileUseCase.execute(session);
    if (supply.userId != profile.id) throw Exception('Acesso negado.');
